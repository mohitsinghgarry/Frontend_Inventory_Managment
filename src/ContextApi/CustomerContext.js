...
export const CustomerProvider = ({ children }) => {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        const response = await fetch('https://backend-inventory-management-1-fxba.onrender.com/products');
        if (!response.ok) {
  ...
  const removeProduct = async (productId) => {
    try {
      const response = await fetch(`https://backend-inventory-management-1-fxba.onrender.com/remove`, { // Assuming your endpoint is set to handle removal via POST
        method: 'POST',
        headers: {
